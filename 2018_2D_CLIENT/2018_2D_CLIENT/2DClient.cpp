...
BITMAP_IMAGE buliding_tile;
BITMAP_IMAGE buliding_tile1;
BITMAP_IMAGE buliding_tile2;
BITMAP_IMAGE buliding_tile3;
BITMAP_IMAGE buliding_tile4;
...
void ProcessPacket(char *ptr)
{
	...
	case SC_POS:
		...
		if (other_id == g_myid) {
			...
			skelaton[other_id].HP = my_packet->HP; //확장 프로토콜
		} else {
			npc[other_id - NPC_START].x = my_packet->x;
			npc[other_id - NPC_START].y = my_packet->y;
			npc[other_id - NPC_START].HP = my_packet->HP; //확장 프로토콜
		}
		break;
		}
...
void SendPacket(int id, void *ptr)
{
	...
	int res = WSASend(g_mysocket, &s_over->m_wsabuf, 1, NULL, 0,
		&s_over->m_over, NULL);
	if (0 != res) {
...
//Create_Bitmap32(&building1, 0, 0, 531, 532);

//클라 추가 코드
Create_Bitmap32(&buliding_tile, 0, 0, 531, 532);
Create_Bitmap32(&buliding_tile1, 0, 0, 531, 532);
Create_Bitmap32(&buliding_tile2, 0, 0, 531, 532);
Create_Bitmap32(&buliding_tile3, 0, 0, 531, 532);
Create_Bitmap32(&buliding_tile4, 0, 0, 531, 532);
//===========================================
Load_Image_Bitmap32(&reactor,L"CHESSMAP2.BMP",0,0,BITMAP_EXTRACT_MODE_ABS);
Load_Image_Bitmap32(&black_tile,L"CHESSMAP2.BMP",0,0,BITMAP_EXTRACT_MODE_ABS);
black_tile.x = 69;
black_tile.y = 5;
black_tile.height = TILE_WIDTH;
black_tile.width = TILE_WIDTH;
Load_Image_Bitmap32(&white_tile,L"CHESSMAP2.BMP",0,0,BITMAP_EXTRACT_MODE_ABS);
white_tile.x = 5;
white_tile.y = 5;
white_tile.height = TILE_WIDTH;
white_tile.width = TILE_WIDTH;

Load_Image_Bitmap32(&buliding_tile, L"CHESSMAP7.BMP", 0, 0, BITMAP_EXTRACT_MODE_ABS);
buliding_tile.x = 5+69+65 + 65 + 65 + 65 + 65;
buliding_tile.y = 5;  //+65 +65+65+65+65+65+65;
buliding_tile.height = TILE_WIDTH;
buliding_tile.width = TILE_WIDTH;

Load_Image_Bitmap32(&buliding_tile1, L"CHESSMAP7.BMP", 0, 0, BITMAP_EXTRACT_MODE_ABS);
buliding_tile1.x = 5 + 69 + 65 + 65 + 65 ;
buliding_tile1.y = 5;  //+65 +65+65+65+65+65+65;
buliding_tile1.height = TILE_WIDTH;
buliding_tile1.width = TILE_WIDTH;

Load_Image_Bitmap32(&buliding_tile2, L"CHESSMAP7.BMP", 0, 0, BITMAP_EXTRACT_MODE_ABS);
buliding_tile2.x = 5 + 69 + 65 ;
buliding_tile2.y = 5;  //+65 +65+65+65+65+65+65;
buliding_tile2.height = TILE_WIDTH;
buliding_tile2.width = TILE_WIDTH;

Load_Image_Bitmap32(&buliding_tile3, L"CHESSMAP7.BMP", 0, 0, BITMAP_EXTRACT_MODE_ABS);
buliding_tile3.x = 5 + 69 + 65 + 65 + 65 + 65 + 65 +65;
buliding_tile3.y = 5;  //+65 +65+65+65+65+65+65;
buliding_tile3.height = TILE_WIDTH;
buliding_tile3.width = TILE_WIDTH;

Load_Image_Bitmap32(&buliding_tile4, L"CHESSMAP7.BMP", 0, 0, BITMAP_EXTRACT_MODE_ABS);
buliding_tile4.x = 5 + 69 + 65 + 65 + 65 + 65 + 65 +65 +65;
buliding_tile4.y = 5;  //+65 +65+65+65+65+65+65;
buliding_tile4.height = TILE_WIDTH;
buliding_tile4.width = TILE_WIDTH;

// now let's load in all the frames for the skelaton!!!

	Load_Texture(L"CHESS4.PNG", UNIT_TEXTURE, 384, 64);
...
// get player input
	...
	for (int i=0; i<10; ++i)
		for (int j=0; j<10; ++j)
		{
			...
			if (((tile_x >> 2) % 3) == ((tile_y >> 2) % 5))
			{
				//Draw_Bitmap32(&buliding_tile2, TILE_WIDTH * i + 7, TILE_WIDTH * j + 7);
				if(i%3 ==0)
					Draw_Bitmap32(&buliding_tile2, TILE_WIDTH * i + 7, TILE_WIDTH * j + 7);
				else if (i % 3 == 1)
					Draw_Bitmap32(&buliding_tile1, TILE_WIDTH * i + 7, TILE_WIDTH * j + 7);
				else if (i % 3 == 2)
					Draw_Bitmap32(&buliding_tile, TILE_WIDTH * i + 7, TILE_WIDTH * j + 7);
			}
			else
